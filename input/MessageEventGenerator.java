...
public class MessageEventGenerator implements EventQueue {
	...
	/**(Mandatory) Fetching the Priority of the messages 
	 * It will be chosen randomly between the range specified
	 * The Sender will be determining the priorits of the messages */
	...
	/** The priority of the messages chosen at the time of creating the message */
	...
	public MessageEventGenerator(Settings s){
		...
		this.priority= s.getCsvInts(Priority);
	...
	/**
	 * Generated a (random) priority of the message
	 * @return priority for the message
	 */
	protected int drawMessagePriority(){
		int priDiff = priority[0]==priority[1]? 0 :rng.nextInt(priority[1]- priority[0]);
		return priority[0]+priDiff;
	}
	...
	public ExternalEvent nextEvent() {
		...
		importance = drawMessagePriority();
		/* Create event and advance to next event */
		MessageCreateEvent mce = new MessageCreateEvent(from, to,importance, this.getID(), msgSize, responseSize, this.nextEventsTime);
