...
public class DistanceDelayReport extends Report implements MessageListener {
	...
	public void messageTransferred(Message m, DTNHost from, DTNHost to,
		...
		report(m.getId(), info.getLoc1().distance(info.getLoc2()),getSimTime() - info.getTime(), m.getHops().size()-1);
	}
	...
	public void newMessage(Message m) {
		if (isWarmup()) {
			...
			Coord  from=m.getFrom().getLocation();
			DTNHost temp = null;
			double large=0.0;
			for( DTNHost host : m.getTo().getAllHosts())
			{
				if(large<=from.distance(host.getLocation()))
					{
						large=from.distance(host.getLocation());
						temp=host;
					}
			}

		this.creationInfos.put( m.getId(), 
				new InfoTuple(getSimTime(), m.getFrom().getLocation().clone(),	temp.getLocation().clone()) );
	}
