...
public class MessageTransferAcceptPolicy {
	...
	public boolean acceptSending(DTNHost from, DTNHost to, Connection con,
		...
		if (! (checkSimplePolicy(m.getTo().containsAddress(to.getAddress()), this.toSendPolicy, myAddr) &&
			checkSimplePolicy(m.getFrom(), this.fromSendPolicy,	myAddr)) ) {
			return false;
		}
		
		if (m.getTo().hasDTNHost(to) && 	!checkHopCountPolicy(m, this.hopCountSendPolicy)){
			return false;
		}
	...
	public boolean acceptReceiving(DTNHost from, DTNHost to, Message m) {
		...
		if (! (checkSimplePolicy(m.getTo().containsAddress(myAddr), this.toReceivePolicy,myAddr) && 
			checkSimplePolicy(m.getFrom(), this.fromReceivePolicy, myAddr)) ) {
			return false;
		}
		
		if (m.getTo().containsAddress(myAddr) != to &&  !checkHopCountPolicy(m, this.hopCountReceivePolicy)) {
			return false;
		} 
