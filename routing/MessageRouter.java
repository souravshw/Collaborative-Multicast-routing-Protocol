...
public abstract class MessageRouter {
	...
	private List<MetaMessage> metadata;
	...
	public void init(DTNHost host, List<MessageListener> mListeners) {
		metadata= new ArrayList<MetaMessage>();
	...
	public void updateMetadata(Message m2){
		
		for(Object msg: metadata.toArray()){
			if(((MetaMessage) msg).getId() == m2.getId())
				return;
		}
		MetaMessage m = new MetaMessage(m2.getLocation(),m2.getId(),m2.getTo().get_gang_id());
		this.metadata.add(m);
	}
	/**
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		isFinalRecipient= this.host.isMemberGang(aMessage.getTo());
...
class MetaMessage{
	...
	public MetaMessage(Coord lo, String msgID, String gangID){
		this.location=lo;
		this.msgID = msgID;
		this.gangID = gangID;
	}
