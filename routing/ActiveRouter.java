...
public abstract class ActiveRouter extends MessageRouter {
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		if (getHost().isMemberGang(m.getTo()) && m.getResponseSize() > 0) {
	...
	protected int startTransfer(Message m, Connection con) {
		...
		else if (deleteDelivered && retVal == DENIED_OLD && 	m.getTo().hasDTNHost( con.getOtherNode(this.getHost()))) {
			/* final recipient has already received the msg -> delete it */
			this.deleteMessage(m.getId(), false);
		}
		con.getOtherNode(getHost()).getRouter().updateMetadata(m);
	...
	protected int checkReceiving(Message m, DTNHost from) {
		...
		if (m.getTtl() <= 0 && m.getTo().hasDTNHost( getHost())) {
		...
		for (Message m : getMessageCollection()) {
			for (Connection con : getConnections()) {
				DTNHost to = con.getOtherNode(getHost());
						if (m.getTo().getAllHosts().contains(to)) {
