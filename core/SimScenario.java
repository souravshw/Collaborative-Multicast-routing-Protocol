...
public class SimScenario implements Serializable {
	...
	/**Package looking for total no. of multicast groups in Scenario */
	private static final String totalGangs="gangs";
	/** total no. of gangs in simulator */
	private int gang;
	...
	protected SimScenario() {
		...
		s.setNameSpace("HostGang");
	this.gang=s.getInt(totalGangs);
	List<DTNHost> member;
	hostGangs = new ArrayList<HostGroup>();
	
	for(int i=0;i<gang;i++)
	{			
		int []adr =  s.getCsvInts(totalGangs+Integer.toString(i));
		member = new ArrayList<DTNHost>();
		for(int x : adr)
			for(DTNHost temp: hosts)
				if(x==temp.getAddress())
					{
						member.add(temp);
					}
		HostGroup temp = new HostGroup(member,totalGangs+Integer.toString(i));
		hostGangs.add(temp);
		member.clear();
	}	
	this.world = new World(hosts,hostGangs,worldSizeX, worldSizeY, updateInterval, 
