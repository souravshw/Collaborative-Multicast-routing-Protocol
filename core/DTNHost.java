...
public class DTNHost implements Comparable<DTNHost> {
	...
	private List<HostGroup> hostGangs;
	//private List<HostGroup> joinableGangs;
	
	private CreateFile msgFile;
	...
	public DTNHost(List<MessageListener> msgLs, List<MovementListener> movLs, 
		...
		msgFile = new CreateFile(this.toString());
		...
		hostGangs = new ArrayList<HostGroup>();
	...
	/*
	 * This will allow the files to write into the self DTN files
	 * about their activities through out  the scenario
	 * @param Line  The Line which is going to be printed.
	 */
		
	/**
	 * Joins the Messaging group
	 * adds those groups in the list which he belongs 
	 */
	public void joinGang(HostGroup hg){
		this.hostGangs.add(hg);
}
	
	public boolean isMemberGang(HostGroup gang){
		return this.hostGangs.contains(gang);
	}
	...
	public void sendMessage(String id, DTNHost to) {
		//fileLine=fileLine.concat(" Sending message "+id+" to "+to.toString()+" at "+SimClock.getTime()+"\n");
		Message m2 = this.router.getMessage(id);
		to.getRouter().updateMetadata(m2);
	...
	//	fileLine=fileLine.concat(" Receiving the message "+m.getId()+" from "+from.toString()+" at "+SimClock.getTime()+"\n");
	...
	public void createNewMessage(Message m) {
		msgLine=msgLine.concat(this.toString()+" "+m.getId()+" "+SimClock.getTime()+" "+this.location.toString()+"\n");
			if(msgLine != "")
			{
				msgFile.WriteIntoFile(msgLine);
				msgLine="";
			}
