...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	...
	protected void connect(Connection con, NetworkInterface anotherInterface) {
		...
		if(! this.timeStamp.containsKey(anotherInterface.getHost()) ||  this.timeStamp.isEmpty())
			{
					...
					this.timeStamp.put(anotherInterface.getHost(), SimClock.getTime());
				}
		else
			{
					timeCheck= this.timeStamp.get(anotherInterface.getHost());
					if((SimClock.getTime() - timeCheck ) > 1800){
								this.timeStamp.remove(anotherInterface.getHost());
